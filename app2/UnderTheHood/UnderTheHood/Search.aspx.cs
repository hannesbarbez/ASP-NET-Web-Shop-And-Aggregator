...
namespace app0
{
    public partial class Search : System.Web.UI.Page
    {
        ...
        protected void lvProducts_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            ...
            UthProduct p = (UthProduct)dataItem.DataItem;
            ...
            prod_man.Text = p.UthManufacter.man_name;
        ...
        protected void lvFeatProd_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            ...
            featProd_man.Text = p.UthManufacter.man_name;
        ...
        private void ShowStats(bool validString, bool validUcid, bool validCid, bool validMid, string searchString, int ucid, int cid, int mid)
        {
            ...
            if (howManyResults > 0)
            {
                ...
                if (validMid) ltStats.Text += ", by " + db.UthManufacters.Single(m => m.man_id == mid).man_name;
                if (validUcid)
                {
                    if (ucid == -1) ltStats.Text += " in all categories";
                    else ltStats.Text += " in category &quot;" + db.UthCategories.Single(c => c.cat_id == ucid).cat_name.ToLower() + "&quot;";
                }
                else if (validCid) ltStats.Text += " in category &quot;" + db.UthCategories.Single(c => c.cat_id == cid).cat_name.ToLower() + "&quot;";
            }
            ltStats.Text += ".";
        }
        ...
        private void ShowResultBasedOnUnsortingCategoryAndManufacter(int ucid, int mid)
        {
            ...
            if (ucid == -1) v = db.UthProducts.Where(p => p.man_id == mid);
        ...
        private void ShowResultBasedOn(int ucid)
        {
            ...
            if (ucid == -1) v = db.UthProducts.Select(p => p);
        ...
        private void ShowOnlyRelevantManufacters(IQueryable<UthProduct> searchResults)
        {
            int[] i = new int[searchResults.Count()];
            int j = 0;

            foreach (UthProduct x in searchResults)
            {
                i[j] = (int)x.man_id;
                j++;
            }
        ...
        private void ShowOnlyRelevantCategories(IQueryable<UthProduct> searchResults)
        {
            int[] i = new int[searchResults.Count()];
            int j = 0;
