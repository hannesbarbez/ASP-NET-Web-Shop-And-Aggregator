...
namespace app0
{
    public partial class Search : System.Web.UI.Page
    {
        ...
        private void InitBlCatNavBar()
        {
            ...
            var v = db.XhCategories.OrderBy(c => c.cat_name);
        ...
        private void InitDdlFilterCategory()
        {
            ...
            ddlFilterCategory.DataSource = db.XhCategories.Select(c => c).OrderBy(c => c.cat_name);
        ...
        private void InitDdlFilterManufacter()
        {
            ...
            ddlFilterManufacter.DataSource = db.XhManufacters.Select(m => m).OrderBy(m => m.man_name);
        ...
        {
            XhDataContext db = new XhDataContext();
            lvFeatProd.DataSource = (from p in db.XhProducts
                                    orderby db.Random()
                                    select p).Take(6);
            lvFeatProd.DataBind();
        }
        ...
        {
            XhDataContext db = new XhDataContext();
            return from c in db.XhCategories
                   orderby c.cat_name
        ...
        protected void lvProducts_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            ...
            prod_man.Text = p.XhManufacter.man_name;
        ...
        protected void lvFeatProd_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            ...
            featProd_man.Text = p.XhManufacter.man_name;
        ...
        private void ShowStats(bool validString, bool validUcid, bool validCid, bool validMid, string searchString, int ucid, int cid, int mid)
        {
            ...
            if (howManyResults > 0)
            {
                ...
                if (validMid) ltStats.Text += ", by " + db.XhManufacters.Single(m => m.man_id == mid).man_name;
                if (validUcid)
                {
                    if (ucid == -1) ltStats.Text += " in all categories";
                    else ltStats.Text += " in category &quot;" + db.XhCategories.Single(c => c.cat_id == ucid).cat_name.ToLower() + "&quot;";
                }
                else if (validCid) ltStats.Text += " in category &quot;" + db.XhCategories.Single(c => c.cat_id == cid).cat_name.ToLower() + "&quot;";
            }
            ltStats.Text += ".";
        }
        ...
        private void ShowAllProducts()
        {
            ...
            var v = db.XhProducts.Select(p => p).OrderBy(p => p.prod_price);
        ...
        {
            XhDataContext db = new XhDataContext();
            var v = db.XhProducts.Where(p => p.cat_id == ucid && p.man_id == mid);
            if (ucid == -1) v = db.XhProducts.Where(p => p.man_id == mid);
        ...
        {
            XhDataContext db = new XhDataContext(); 
            var v = db.XhProducts.Where(p => p.cat_id == ucid);
            if (ucid == -1) v = db.XhProducts.Select(p => p);
        ...
        private void ShowOnlyRelevantManufacters(IQueryable<XhProduct> searchResults)
        {
            int[] i = new int[searchResults.Count()];
            int j = 0;

            foreach (XhProduct x in searchResults)
            {
                i[j] = (int)x.man_id;
                j++;
            }
        ...
        private void ShowOnlyRelevantCategories(IQueryable<XhProduct> searchResults)
        {
            int[] i = new int[searchResults.Count()];
            int j = 0;
